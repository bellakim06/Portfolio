...
void Game::playGame(bool isAIModeIn, ifstream& gameFile) {
     ...
     */
    string inputString;
    while (gameFile >> inputString) {
        Person person(inputString);
        int tickspawned = person.getTurn();
        
        while (building.getTime() < tickspawned) {
    ...
    bool Game::isValidPickupList(const string& pickupList,
        ...
        for (int k = 0; k < pickupList.size(); k++) {
            if ((pickupList.at(k) - '0') < 0 || (pickupList.at(k) - '0') > 9) {
                return false;
            }
        }
        ...
        Floor fl = building.getFloorByFloorNum(pickupFloorNum);
        
        for (int h = 0; h < pickupList.size(); h++) {
            if (pickupList.at(h)-'0' >= fl.getNumPeople()) {
                return false;
            }
        }
        
        int firstTargetFloor = fl.getPersonByIndex(pickupList.at(0) - '0').getTargetFloor();
        char targetDirection;
        if (fl.getPersonByIndex(pickupList.at(0) - '0').getTargetFloor() > pickupFloorNum) {
            targetDirection = 'u';
        } else {
            targetDirection = 'd';
        }
        ...
        char direction = 'a';
        for (int o = 0; o < pickupList.size(); o++) {
            int index = pickupList.at(o) - '0';
            if ((fl.getPersonByIndex(index).getTargetFloor()) > pickupFloorNum)
            {
                direction = 'u';
            }
            else {
